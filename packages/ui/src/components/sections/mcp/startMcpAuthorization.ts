    ...
    if (!applied.ok) {
      throw new McpAuthorizationError(
        applied.requiresManualRestart
          ? 'The callback settings changed, but pi must be restarted manually before authorization can start.'
      ...
      if (existing && currentOAuth?.redirectUri !== desiredRedirectUri) {
        ...
        if (saved.restartDeferred) {
          ...
          if (applied.requiresManualRestart) {
            throw new McpAuthorizationError(
              'The callback URL was saved, but pi must be restarted manually before authorization can start.',
            );
          }
          if (!applied.ok) {
            throw new McpAuthorizationError(
