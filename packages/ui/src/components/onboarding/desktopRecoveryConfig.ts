...
export function getDesktopRecoveryConfig(
  ...
  switch (variant) {
    case 'local-unavailable':
      return {
        title: 'Local pi Unavailable',
        description: 'pi CLI could not be started or is not installed. Install pi or connect to a remote server instead.',
    ...
    case 'remote-wrong-service': {
      ...
      return {
        ...
        description: `The server at "${host || 'unknown'}" is not running pichamber. Verify the address points to an pichamber server.`,
    ...
    case 'remote-incompatible': {
      ...
      return {
        ...
        description: `The pichamber server at "${host || 'unknown'}" is not compatible with this app version. Update pichamber on the server, then try again.`,
