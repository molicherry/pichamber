...
const ensureAbsoluteBaseUrl = (candidate: string): string => {
  ...
  try {
    ...
    console.warn("Failed to normalize pi base URL:", error);
    return normalized;
  }
};
    ...
    fetch: async (input: string | URL | Request, init?: RequestInit) => {
      ...
      try {
        ...
        if (timeout.signal.aborted && !callerSignal?.aborted) {
          throw new Error(`pi request timed out after ${requestTimeoutMs}ms`);
        }
        throw error;
      } finally {
        detachFallback?.();
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const message =
          typeof error.error === 'string' && error.error.length > 0
            ? error.error
            : 'Failed to update pi working directory';
        throw new Error(message);
      }
      ...
      console.warn('Failed to update pi working directory:', error);
      throw error;
    }
  }
}
