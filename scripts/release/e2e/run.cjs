...
for (const id of selected) {
  ...
  const result = spawnSync("bun", [script], { stdio: "inherit", timeout: 180_000 });
  const durationMs = Date.now() - started;
  results.push({ id, status: result.status === 0 ? "passed" : "failed", durationMs, exitCode: result.status, error: result.error?.message });
}

const passed = results.filter((r) => r.status === "passed");
const failed = results.filter((r) => r.status !== "passed");
console.log("\n=== P0 UI E2E summary ===");
for (const r of results) console.log(`${r.status.padEnd(8)} ${r.id}${r.durationMs ? ` (${r.durationMs}ms)` : ""}${r.error ? ` — ${r.error}` : ""}`);
