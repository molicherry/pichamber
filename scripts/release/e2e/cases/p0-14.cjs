...
async function main() {
  ...
  try {
    ...
    await h.page.getByText(REPLY_QUESTION_DISMISSED, { exact: false }).first().waitFor({ state: "visible", timeout: 20_000 });
    await h.assertClean();
...
main().catch((error) => {
  console.error(`UI-P0-14 failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exit(1);
});
