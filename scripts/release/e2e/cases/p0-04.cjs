...
async function main() {
  ...
  try {
    ...
    await h.page.getByText(REPLY_STREAM, { exact: false }).first().waitFor({ state: "visible", timeout: 20_000 });

    if (h.registry.runtimesList().length !== 1) {
      throw new Error(`expected exactly one session after first send, got ${h.registry.runtimesList().length}`);
    }
...
main().catch((error) => {
  console.error(`UI-P0-04 failed: ${error instanceof Error ? error.message : String(error)}`);
  process.exit(1);
});
